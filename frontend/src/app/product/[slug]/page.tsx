    ...
    <div className="min-h-screen bg-[#F8F6F2]">
      ...
      <main className="max-w-[1200px] mx-auto px-4 py-12">
        ...
        <div className="flex items-center gap-2 text-[11px] text-[#B8B4AE] mb-10">
          <Link href="/main" className="hover:text-[#111] transition-colors">
            홈
          </Link>
          <span>/</span>
          <Link
            href={`/category/${encodeURIComponent(product.mainCategory ?? "")}`}
        ...
        <div className="grid grid-cols-2 gap-16">
          ...
          {/* 정보 */}
          <div>
            <p className="text-[11px] tracking-[0.12em] uppercase text-[#B8B4AE] mb-2">
              {product.brand}
            </p>
            <h1 className="text-[26px] font-light tracking-[-0.02em] text-[#111] mb-4 leading-snug">
              {product.name}
            </h1>
            ...
            <div className="flex items-center gap-1.5 mb-6">
              {Array.from({ length: 5 }).map((_, i) => (
                <svg
                  key={i}
                  className={`w-3 h-3 ${i < Math.floor(product.rating ?? 0) ? "text-[#1F3D2A]" : "text-[#D8D4CE]"}`}
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
              ...
              <span className="text-[12px] text-[#888480]">
                {product.rating} · {product.purchaseCount?.toLocaleString()}개
                구매
              </span>
            </div>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    {product.discountRate && (
                      <span className="text-[13px] text-[#B84A28] font-medium">
                        {product.discountRate}%
                      </span>
                    )}
                    <span className="text-[28px] font-semibold text-[#111] tracking-[-0.02em]">
                      {product.salePrice.toLocaleString()}원
                    </span>
                  </div>
                  <span className="text-[14px] text-[#B8B4AE] line-through">
            ...
            <div className="flex items-center justify-between pb-6 border-b border-[#D8D4CE] mb-6">
              ...
              <div className="flex items-center gap-4">
                <button
                  ...
                  className="w-7 h-7 border border-[#D8D4CE] text-[#888480] hover:border-[#111] hover:text-[#111] transition-colors text-sm"
                >
                  −
                </button>
            ...
            <div className="flex items-center justify-between mb-8">
              ...
              <span className="text-[22px] font-semibold text-[#111] tracking-[-0.02em]">
                {totalPrice}원
              </span>
            </div>
